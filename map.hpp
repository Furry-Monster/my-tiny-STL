...
public:
  ...
  template <class _Kv, _LIBPENGCXX_REQUIRES_TRANSPARENT_COMPARE(_ValueComp, _Kv,
                                                                value_type)>
...
public:
  ...
  explicit multi_map(std::initializer_list<value_type> __ilist, _Compare __comp)
      : _RbTreeImpl<value_type, _ValueComp, _Alloc>(__comp) {
    _M_multi_insert(__ilist.begin(), __ilist.end());
  }
  ...
  template <class... _Ts> std::pair<iterator, bool> emplace(_Ts &&...__value) {
